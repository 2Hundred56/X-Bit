...
TileHandle::TileHandle(Tile *t, Vector p) {
	tile = t;
	pos = p;
}
...
unsigned char TileHandle::GetSortingFlag() {
	return tile->GetSortingFlag();
}

unsigned char TileHandle::GetTypeFlag() {
	return tile->GetTypeFlag();
}
